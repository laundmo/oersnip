...
class EnvironmentNotFound(Exception):
    def __init__(self, environment=None, snippet_name=None):
        super().__init__(
            f"Missing environment '{environment}' for snippet '{snippet_name}'"
        )
        ...
        for environment in self.required_envs:
            if environment not in envs.keys():
                raise EnvironmentNotFound(
                    environment=environment, snippet_name=self.name
                )
        return {
            key: value
...
class SnippetsSearch:
    ...
    def search_snippet(self, term):
        ...
        results = process.extract(
            term,
            snippet_choice_pair.keys(),
            scorer=fuzz.partial_token_sort_ratio,
            score_cutoff=20,
        )

        # Get snippets instead of search term...
        results = [(snippet_choice_pair[r[0]], *r[1:]) for r in results]
